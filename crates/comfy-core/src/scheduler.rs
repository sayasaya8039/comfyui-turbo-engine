...
impl DeviceScheduler {
    ...
    pub fn execute(&self, json: &str) -> ComfyResult<HashMap<String, NodeOutputs>> {
        ...
        let all_outputs = Arc::new(Mutex::new(HashMap::<String, NodeOutputs>::new()));
        let stats = Arc::new(Mutex::new(SchedulerStats::default()));

        // Execute each depth group (groups are sequential; nodes within a group are parallel)
        for group in &depth_groups {
            if group.len() == 1 {
                // Single node — execute directly without rayon overhead
                let node_id = &group[0];
                ...
                // Track device hint
                {
                    ...
                    s.total_nodes += 1;
                    match device {
                        Device::Gpu(_) => s.gpu_nodes += 1,
                        Device::Npu(_) => s.npu_nodes += 1,
                        Device::Cpu => s.cpu_nodes += 1,
                    }
                }
                ...
                let mut inputs = NodeInputs::new();
                {
                    ...
                    for (input_name, input_val) in &workflow_node.inputs {
                        let resolved = Self::resolve_input(input_val, &outputs_guard)?;
        ...
        let final_outputs = Arc::try_unwrap(all_outputs)
            .unwrap_or_else(|arc| {
        ...
        if let Ok(mut s) = self.stats.lock() {
            *s = Arc::try_unwrap(stats)
                .unwrap_or_else(|_| Mutex::new(SchedulerStats::default()))
                .into_inner()
                .unwrap();
        }
